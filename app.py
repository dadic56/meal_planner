...
@socketio.on('edit_plat')
def handle_edit_plat(data):
    plat = data.get('plat')
    ...
    emit('update_selection', [selection.plat for selection in Selection.query.all()], broadcast=True)

@socketio.on('delete_plat')
def handle_delete_plat(data):
    plat = data.get('plat')
