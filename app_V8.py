...
def get_ingredients():
    ...
    if plat:
        ingredients = df[df['Plat'] == plat][['Ingrédient', 'Quantité']].to_dict(orient='records')
...
def handle_generer_liste(data):
    ...
    selected_plats = data  # Utiliser les plats sélectionnés envoyés par le client
...
def handle_ajouter_plat(data):
    ...
    if plat and ingredients:
        for ingredient in ingredients:
            df.loc[len(df)] = [plat, ingredient['ingredient'], f"{ingredient['quantite']} {ingredient['unite']}"]
